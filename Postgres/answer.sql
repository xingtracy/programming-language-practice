...
-- Write an SQL query to retrieve all products in a specific category.
	...
	SELECT product_id, product_name, description, price, category_id
...
-- Hint: You may need to use subqueries or common table expressions (CTEs) to solve this problem.
  ...
  having AVG(CAST(r.rating AS float)) = 
      ( select AVG(CAST(r.rating AS float)) as AverageRate from Products p
        join Reviews r on r.product_id = p.product_id
        group by p.product_id, p.product_name
...
-- Hint: You may need to use subqueries or window functions to solve this problem.
  ...
  where  (o2.order_date - o.order_date) = 1  and userOrder.orderCount = 1
...
-- Hint: You may need to use subqueries, joins, and aggregate functions to solve this problem.
  ...
  select sum(o.total_amount) as TotalAmount, c.category_id,c.category_name 
  from  Orders o
  join Order_Items oi on oi.order_id = o.order_id
  join Products p on p.product_id = oi.product_id
...
-- Hint: You may need to use subqueries, joins, and window functions to solve this problem.
  ...
  (
      ...
      join Users u on u.user_id = o.user_id
      join (select count(1) as orderCount, user_id, order_date from Orders group by user_id, order_date) as userOrder on userOrder.user_id = o.user_id and userOrder.order_date = o.order_date
      where  (o2.order_date - o.order_date) = 1  
  ) as ud
...
--Online https://datalemur.com/blog/shopify-sql-interview-questions
